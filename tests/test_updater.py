...
def test_send_agg_metrics(domains_config, updater_metrics, cwm_api_manager):
    ...
    with domains_config.get_redis() as r:
        r.set("worker:aggregated-metrics:{}".format(domain_name), json.dumps({
            'lu': "20210302030405",
            'm': [{'t': datetime.datetime(2021,3,2,1,2,5).astimezone(pytz.UTC).strftime('%Y%m%d%H%M%S'), 'bytes_in': '5000', 'bytes_out': '40000', 'num_requests_in': '1000', 'num_requests_out': '7000', 'num_requests_misc': '500', "cpu_seconds": "1234.5", "ram_bytes": "5678"},
    ...
    with domains_config.get_redis() as r:
        last_sent_update = (datetime.datetime.now(pytz.UTC) - datetime.timedelta(seconds=61)).strftime('%Y%m%d%H%M%S')
...
def test_updater_daemon(domains_config, deployments_manager, updater_metrics, cwm_api_manager):
    ...
    updated_more_then_half_hour_ago = (datetime.datetime.now(pytz.UTC) - datetime.timedelta(minutes=35)).strftime("%Y-%m-%d %H:%M:%S")
    updated_less_then_half_hour_ago = (datetime.datetime.now(pytz.UTC) - datetime.timedelta(minutes=25)).strftime("%Y-%m-%d %H:%M:%S")
    updated_less_then_day_ago = (datetime.datetime.now(pytz.UTC) - datetime.timedelta(hours=23)).strftime("%Y-%m-%d %H:%M:%S")
    updated_more_then_day_ago = (datetime.datetime.now(pytz.UTC) - datetime.timedelta(hours=25)).strftime("%Y-%m-%d %H:%M:%S")
    updated_less_then_hour_ago = (datetime.datetime.now(pytz.UTC) - datetime.timedelta(minutes=55)).strftime("%Y-%m-%d %H:%M:%S")
    updated_more_then_hour_ago = (datetime.datetime.now(pytz.UTC) - datetime.timedelta(minutes=65)).strftime("%Y-%m-%d %H:%M:%S")
    ...
    with domains_config.get_redis() as r:
        for namespace_name, last_action in {
            "deployed--has--action--recent-update": datetime.datetime.now(pytz.UTC) - datetime.timedelta(minutes=25),
            "deployed--has--action--old-update": datetime.datetime.now(pytz.UTC) - datetime.timedelta(minutes=25)
        }.items():
